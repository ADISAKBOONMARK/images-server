import { EXPECT } from '../../MainProperty.test';
import { LOG } from '../../../src/MainProperty';

import Model from '../../../src/Models/Image/ImageModel';

const service = 'remove';

const runTest = async function (controller) {
    describe(service, async () => {
        it('success basic flow', async function () {
            const log = LOG;
            const params = {
                group: 'test',
                base64:
                    'data:image/png;base64,',
            };

            const model = new Model();

            let result = await model.import(log, params);

            EXPECT(result.code).to.equal(202);

            params.id = result.data.id;

            result = await model.remove(log, params);

            EXPECT(result.code).to.equal(202);
        });

        it('image not found', async function () {
            const log = LOG;
            const params = {
                group: 'test',
                id: '123456789',
            };

            const model = new Model();

            const result = await model.remove(log, params);

            EXPECT(result.code).to.equal(404);
        });

        it('validate fail', async function () {
            const log = LOG;
            const params = {};

            const model = new Model();

            const result = await model.remove(log, params);

            EXPECT(result.code).to.equal(400);
        });
    });
};

export default { runTest };
